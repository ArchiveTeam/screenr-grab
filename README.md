...
More information about the archiving project can be found on the ArchiveTeam wiki: [screenr](http://archiveteam.org/index.php?title=screenr)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "screenr" project in the Warrior interface.
...
### For Debian/Ubuntu:
    ...
    pip install --upgrade seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/screenr-grab.git; cd screenr-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/screenr-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, screenr-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/screenr-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
